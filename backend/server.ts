...
function createApp(
  ...
  // Error handling must be registered after all routes so Express recognizes it
  // as an error-handling middleware (4-arg) and next(err) reaches it.
  app.use((err: Error, req: Request, res: Response, _next: NextFunction) => {
    const requestId = req.context?.requestId ?? 'unknown';
    const timestamp = req.context?.timestamp ?? new Date();
