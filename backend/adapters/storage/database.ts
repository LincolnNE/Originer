...
export class DatabaseStorageAdapter implements StorageAdapter {
  ...
  async loadMessages(messageIds: string[]): Promise<Message[]> {
    ...
    // Preserve session conversation order (messageIds sequence), not creation time.
    // ORDER BY created_at would reorder turns incorrectly when learner + instructor
    // messages share the same millisecond or when clocks skew.
    const rowById = new Map(rows.map(row => [row.id as string, row]));

    return messageIds.map(id => {
      const row = rowById.get(id);
      if (!row) {
        throw new Error(`Message record missing for id referenced by session: ${id}`);
      }
      return {
        id: row.id,
        sessionId: row.session_id,
