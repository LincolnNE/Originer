...
export class DatabaseStorageAdapter implements StorageAdapter {
  ...
  async loadMessages(messageIds: string[]): Promise<Message[]> {
    ...
    // Preserve session order (messageIds / sequence_order), not created_at. Same unix
    // timestamps from rapid inserts can sort incorrectly and scramble conversation history.
    const byId = new Map<string, (typeof rows)[0]>(rows.map((row) => [row.id, row]));
    return messageIds
      .map((id) => byId.get(id))
      .filter((row): row is (typeof rows)[0] => row != null)
      .map((row) => ({
        id: row.id,
        sessionId: row.session_id,
        role: row.role as MessageRole,
