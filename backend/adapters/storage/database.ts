...
export class DatabaseStorageAdapter implements StorageAdapter {
  ...
  async loadMessages(messageIds: string[]): Promise<Message[]> {
    ...
    // Preserve session message order (session_messages.sequence_order). Ordering only by
    // created_at breaks when multiple messages share the same millisecond — which happens
    // for back-to-back learner/instructor turns because message IDs use Date.now().
    return messageIds
      .map(id => byId.get(id))
