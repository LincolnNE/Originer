...
export class DatabaseStorageAdapter implements StorageAdapter {
  ...
  async saveSession(session: Session): Promise<void> {
    ...
    // One transaction: avoid leaving session_messages empty if we crash after DELETE
    const persistSession = this.db.transaction((sess: Session) => {
      stmt.run(
        sess.id,
        sess.instructorId,
        sess.learnerId,
      ...
      sess.messageIds.forEach((messageId, idx) => {
        insertStmt.run(sess.id, messageId, idx);
      });
    });
  ...
  async updateSession(sessionId: string, updates: Partial<Session>): Promise<void> {
    ...
    // Single transaction: junction DELETE + INSERT must not commit without each other
