...
export class DatabaseStorageAdapter implements StorageAdapter {
  ...
  async loadMessages(messageIds: string[]): Promise<Message[]> {
    ...
    // Preserve session order: IN (...) has no reliable order; created_at can tie for
    // messages in the same second, which would scramble conversation history in prompts.
    const byId = new Map(
      rows.map((row) => [row.id as string, row] as const)
    );
    return messageIds.map((id) => {
      const row = byId.get(id);
      if (!row) {
        throw new Error(`loadMessages: message id not in database: ${id}`);
      }
      return {
        id: row.id,
        sessionId: row.session_id,
