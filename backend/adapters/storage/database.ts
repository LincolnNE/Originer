...
export class DatabaseStorageAdapter implements StorageAdapter {
  ...
  async loadMessages(messageIds: string[]): Promise<Message[]> {
    ...
    // Load by id set only — ordering must follow session message order, not created_at
    // (fast same-ms inserts or clock skew can reorder by timestamp and corrupt prompts).
    const rows = this.db
