...
export class DatabaseStorageAdapter implements StorageAdapter {
  ...
  async loadMessages(messageIds: string[]): Promise<Message[]> {
    ...
    // Preserve session order (sequence_order in session_messages), not created_at. Same-ms
    // timestamps or backdated rows would otherwise reorder history and corrupt prompts.
