...
export class DatabaseStorageAdapter implements StorageAdapter {
  ...
  async saveSession(session: Session): Promise<void> {
    // Single transaction: avoid orphaned session_messages or empty index if the process crashes mid-write.
  ...
  async updateSession(sessionId: string, updates: Partial<Session>): Promise<void> {
    ...
    const hasMessageUpdate = updates.messageIds !== undefined;
    ...
    // One transaction: session row updates and session_messages must not diverge on crash/OOM.
    const apply = this.db.transaction(() => {
      if (hasMessageUpdate && updates.messageIds) {
