...
/** Body shape used by frontend `sessionsApi.createSession` / useSession */
...
function newLearnerId(): string {
  return `learn_${Date.now()}_${Math.random().toString(36).slice(2, 11)}`;
}
...
export async function registerSessionRoutes(
  ...
  /**
   * GET /sessions/:id
   * Load session (used by frontend sessionsApi.getSession)
   */
  ...
  /**
   * POST /sessions
   * Create session (legacy path used by frontend sessionsApi.createSession)
   */
  server.post<{ Body: CreateSessionRequestBody }>(
    '/api/v1/sessions',
    async (request: FastifyRequest<{ Body: CreateSessionRequestBody }>, reply: FastifyReply) => {
      const { instructorProfileId, subject, topic, learningObjective, learnerId } = request.body || ({} as CreateSessionRequestBody);

      if (!instructorProfileId || !subject || !topic || learningObjective === undefined || learningObjective === '') {
        return reply.code(400).send({
          success: false,
          error: {
            code: 'INVALID_REQUEST',
            message: 'Missing required fields: instructorProfileId, subject, topic, learningObjective',
          },
        });
      }

      const instructor_id = instructorProfileId;
      const learner_id = learnerId && learnerId.length > 0 ? learnerId : newLearnerId();
      ...
      try {
        ...
        const session = {
          ...
          instructorProfileId: instructor_id,
          subject,
          topic,
          learningObjective,
