...
/** Frontend / API-contract shape (camelCase) — see frontend/types/api.ts */
...
async function ensureLearnerRow(db: DatabaseStorageAdapter, learnerId: string): Promise<void> {
  try {
    await db.createLearner({
      id: learnerId,
      name: learnerId === 'anonymous' ? 'Anonymous Learner' : `Learner ${learnerId}`,
      level: 'beginner',
    });
  } catch (e) {
    if (!isUniqueConstraintError(e)) throw e;
...
function sessionToRestPayload(session: {
  id: string;
  learnerId: string;
  instructorProfileId: string;
  subject: string;
...
export async function registerSessionRoutes(
  ...
  /**
   * POST /sessions
   * Create session (REST contract used by Next.js app and frontend API client)
   */
  server.post<{ Body: CreateSessionRestBody }>(
    ...
    async (request: FastifyRequest<{ Body: CreateSessionRestBody }>, reply: FastifyReply) => {
      ...
      try {
        await ensureInstructorRow(dbAdapter, instructorProfileId);
        const resolvedLearnerId = learnerId || 'anonymous';
        await ensureLearnerRow(dbAdapter, resolvedLearnerId);
    ...
    async (request: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) => {
      ...
      try {
        ...
        return reply.send({
          success: true,
          data: {
            session: sessionToRestPayload(session),
          },
        });
      } catch (error: any) {
        request.log.error(error);
  ...
  server.post<{ Body: StartSessionRequest }>(
    ...
    async (request: FastifyRequest<{ Body: StartSessionRequest }>, reply: FastifyReply) => {
      ...
      try {
        await ensureInstructorRow(dbAdapter, instructor_id);
        await ensureLearnerRow(dbAdapter, learner_id);
