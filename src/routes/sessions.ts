...
/** Body for POST /api/v1/sessions (frontend / API client contract) */
...
export async function registerSessionRoutes(
   ...
   * Create session (same behavior as /start; shape matches frontend apiClient)
   */
  server.post<{ Body: CreateSessionRequest }>(
    ...
    async (request: FastifyRequest<{ Body: CreateSessionRequest }>, reply: FastifyReply) => {
      ...
      const resolvedLearnerId = learnerId || `learner_anon_${Date.now()}`;
      const sessionId = `sess_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;

      try {
        await db.ensureInstructorExists(instructorProfileId);
        await db.ensureLearnerExists(resolvedLearnerId);
   ...
   * Load session for client hooks (useSession)
   */
  ...
  server.post<{ Body: StartSessionRequest }>(
    ...
    async (request: FastifyRequest<{ Body: StartSessionRequest }>, reply: FastifyReply) => {
      ...
      try {
        ...
        await db.ensureInstructorExists(instructor_id);
        await db.ensureLearnerExists(learner_id);
