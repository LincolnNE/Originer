...
import { Session, SessionState } from '../../backend/core/types';

/** Body for POST /api/v1/sessions (frontend + API contract) */
interface CreateSessionV1Request {
...
export async function registerSessionRoutes(
   ...
   * Create a session (used by the Next.js server action and client API)
   */
  server.post<{ Body: CreateSessionV1Request }>(
    '/api/v1/sessions',
    async (request: FastifyRequest<{ Body: CreateSessionV1Request }>, reply: FastifyReply) => {
      const {
        instructorProfileId,
        subject,
        topic,
      ...
      } = request.body || ({} as CreateSessionV1Request);
      ...
      try {
        ...
        const learnerId =
          bodyLearnerId ||
          `learner_anon_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
   ...
   * Load session (used by the client session store)
   */
